<template>
  <div class="yin-header">
    ...
    <div class="header-logo" @click="goPage()">
      <yin-icon class="icon" :icon="iconList.ERJI"></yin-icon>
      <span>{{ musicName }}</span>
    </div>
    ...
    <el-dropdown class="user-wrap" v-if="token" trigger="click">
      <el-image class="user" fit="contain" :src="'/serve'+userPic"   />
      <template #dropdown>
        <el-dropdown-menu>
  ...
  setup() {
    ...
    function goMenuList(path) {
      if (path == RouterName.SignOut) {
        ...
        HttpManager.getState(0,proxy.$store.getters.userId);
...
.yin-header {
  ...
  background:
  linear-gradient(217deg,rgba(80, 157, 136, 0.8), rgba(255,0,0,0) 70.71%),
  linear-gradient(336deg, rgba(68, 68, 121, 0.8), rgba(0,0,255,0) 70.71%),
  linear-gradient(127deg, rgba(220, 92, 152, 0.8), rgba(0,255,0,0) 70.71%);
