...
export default defineComponent({
  ...
  setup() {
    ...
    //   isCollection.value = (
    //     (await HttpManager.isCollection(params)) as ResponseBody
    //   ).data;
    // }
    Querysongs();

    async function Querysongs() {
      let userId = proxy.$store.state.user.userId;
      HttpManager.getsearchforCollectsongs(userId, songId.value).then((res) => {
        isCollections.value = res as ResponseBody;
        if (isCollections.value == "1") {
    ...
    async function changeCollection() {
      ...
      let userId = proxy.$store.state.user.userId;
      HttpManager.setCollection(userId, songId.value).then((res) => {
        isCollection.value = true;
      });
  ...
  methods: {
    ...
    async prev() {
      if (this.playState === Icon.LUANXU) {
        ...
        console.log(this.currentPlayList[playIndex].url);
      } else if (
        ...
        if (this.currentPlayIndex > 0) {
          ...
          const resultSong = (await HttpManager.getSongPlay(
            this.currentPlayList[this.currentPlayIndex].id
          )) as ResponseBody;

          const url = resultSong.data[0].url;
          this.toPlay(url);
          if (this.routeUrl.currentRoute.name == "lyric") {
            location.reload();
          }
        } else {
          this.$store.commit(
          ...
          console.log(this.currentPlayList[this.currentPlayIndex].url);
        }
      }
    }, // 下一首
    async next() {
      if (this.playState === Icon.LUANXU) {
        ...
        if (this.currentPlayIndex < this.currentPlayList.length - 1) {
          this.$store.commit("setCurrentPlayIndex", this.currentPlayIndex + 1);
          console.log(this.currentPlayList[this.currentPlayIndex]);
          const resultSong = (await HttpManager.getSongPlay(
            this.currentPlayList[this.currentPlayIndex].id
          )) as ResponseBody;
          const url = resultSong.data[0].url;
