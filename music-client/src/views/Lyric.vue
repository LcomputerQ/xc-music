...
export default defineComponent({
  ...
  setup() {
    ...
    const songTitle = computed(() => store.getters.singerName); // 歌名
    const singerName = computed(() => store.getters.songTitle); // 歌手名
    const songPic = computed(() => store.getters.songPic); // 歌曲图片
    
    HttpManager.getLyric(songId.value).then((res) => {
    // console.log("lyric = " + lyric.value);
    lyric = (res as ResponseBody).lrc.lyric;
    watch(songId, () => {
    ...
    lyricArr.value = lyric ? parseLyric(lyric) : [];
    })// 歌词
