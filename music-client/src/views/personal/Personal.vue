...
export default defineComponent({
	...
	setup() {
		...
		async function getCollection(userId) {
			...
			const collectIDList = ref([]); // 存放收藏的歌曲ID
			const result = (await HttpManager.getCollectionOfUser(
				userId
			)) as ResponseBody;
			for (let i in result) {
				collectIDList.value.push(result[i].songId);
			}
			const results = (await HttpManager.getSongOfId(
				collectIDList.value.toString()
			)) as ResponseBody;
			for (let i in results.songs) {
				collectSongList.value.push(results.songs[i]);
			}
		}
		function changeData() {
