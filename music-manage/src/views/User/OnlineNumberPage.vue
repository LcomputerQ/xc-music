    ...
    <el-table height="550px" border size="small" :data="data" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用户头像" width="102" align="center">
        <template v-slot="scope">
          <img :src="'/serve/'+scope.row.avator" style="width: 80px" />
        </template>
      </el-table-column>
...
export default defineComponent({
  ...
  setup() {
    ...
    async function getData() {
      tableData.value = [];
      tempDate.value = [];
      const result = (await HttpManager.getOnline()) as ResponseBody;
      console.log(result);
      for(let i in result) {
        tableData.value.push (result[i]) ;
        tempDate.value.push (result[i]) ;
      }
      currentPage.value = 1;
    }
    // 获取当前页
